...
io.on("connection", (socket) => {
    ...
    socket.on(SocketEvent.SEND_OFFER, ({ offer, to }) => {
        io.to(to).emit(SocketEvent.RECEIVE_OFFER, { offer, from: socket.id });
    });

    socket.on(SocketEvent.SEND_ANSWER, ({ answer, to }) => {
        io.to(to).emit(SocketEvent.RECEIVE_ANSWER, { answer, from: socket.id });
    });

    socket.on(SocketEvent.SEND_ICE_CANDIDATE, ({ candidate, to }) => {
        io.to(to).emit(SocketEvent.RECEIVE_ICE_CANDIDATE, { candidate, from: socket.id });
    });

    // Handle reception of WebRTC signaling messages
    socket.on(SocketEvent.RECEIVE_OFFER, ({ offer, from }) => {
        io.to(from).emit(SocketEvent.SEND_OFFER, { offer, to: socket.id });
    });

    socket.on(SocketEvent.RECEIVE_ANSWER, ({ answer, from }) => {
        io.to(from).emit(SocketEvent.SEND_ANSWER, { answer, to: socket.id });
    });

    socket.on(SocketEvent.RECEIVE_ICE_CANDIDATE, ({ candidate, from }) => {
        io.to(from).emit(SocketEvent.SEND_ICE_CANDIDATE, { candidate, to: socket.id });
    });
